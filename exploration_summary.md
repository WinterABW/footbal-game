...
### Approaches
1. **Direct Mapping with Defaults** — Map API fields directly to player.model fields and assign sensible defaults for missing fields
   - Pros: Simple, immediate implementation
   - Cons: Loss of some player variety (all players same level, age, etc.)
   - Effort: Low

2. **Enrichment Lookup Table** — Map core API fields and use a lookup table to add missing attributes based on player ID/name
   - Pros: Preserves existing player characteristics
   - Cons: Requires maintaining lookup table, duplication of data
   - Effort: Medium

3. **Backend Enhancement** — Modify API to return all required fields
   - Pros: Cleanest separation, single source of truth
   - Cons: Requires backend changes, may not be feasible
   - Effort: High

4. **Hybrid Approach** — Fetch basic player data from API and enrich with additional data from localStorage/defaults for purchased players
